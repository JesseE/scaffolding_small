...
test('external flag for node modules', function(t) {
    ...
    var b = fromArgs([ __dirname + '/external_args/index.js', '-x', 'backbone' ]);
    b.bundle(function (err, src) {
        t.ifError(err);
...
test('entry expose', function (t) {
    ...
    var b = fromArgs([
        path.join(__dirname, '/entry_expose/index.js'),
        '--require', path.join(__dirname, '/entry_expose/index.js') + ':x',
    ]);
    b.bundle(function (err, src) {
        t.ifError(err);
