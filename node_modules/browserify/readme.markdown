...
Whip up a file, `index.js` with some `require()`s in it. You can use relative
...
Now just use the `browserify` command to build a bundle starting at `index.js`:

```
$ browserify index.js > bundle.js
```

All of the modules that `index.js` needs are included in the `bundle.js` from a
...
```
$ browserify index.js --debug | exorcist bundle.js.map > bundle.js
```
...
$ browserify index.js --standalone Foo | derequire > bundle.js
```
...
```
$ browserify -t [ foo --bar=555 ] index.js
```
